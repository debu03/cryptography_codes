...
{ 
    ...
    cout << "\n\nTotal " << plaintext_message.length() << " letters including 
blank space -> " << plaintext_message.length() << "*8 = " << total_bits << " 
bits" << endl; 
...
endl; 
    ...
    string Mesaage_With_Length_Bits = padding_output; 
    for (int j = 1; j <= length_bits - 8; j++) 
    { 
        Mesaage_With_Length_Bits += "0"; 
    } 
    Mesaage_With_Length_Bits += int_to_binary(total_bits); 
 
    for (size_t i = 0; i < Mesaage_With_Length_Bits.size(); ++i) 
    { 
        cout << Mesaage_With_Length_Bits[i]; 
        // Add space after every 8 bits 
        if ((i + 1) % 8 == 0) 
        { 
            cout << ' '; 
